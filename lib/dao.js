    ...
    if (typeof key === "object") {
      ...
      if (options.raw && !(this.options && this.options.include) && !this._hasBooleanAttributes) {
        ...
        _.each(values, function(value, key) {
          if (self.daoFactory.rawAttributes[key] && self.daoFactory.rawAttributes[key].type == 'JSON') {
            self.dataValues[key] = value ? JSON.parse(value) : (self.daoFactory.rawAttributes[key].defaultValue || null);
          }
        });
      ...
      if (!options.raw && this._customSetters[key]) {
        ...
        if (this.options && this.options.include && this.options.includeNames.indexOf(key) !== -1) {
          ...
          // Parse JSON elements
          if (this.daoFactory.rawAttributes[key].type == 'JSON' && !_.isObject(value)) {
            value = value ? JSON.parse(value) : (self.daoFactory.rawAttributes[key].defaultValue || null);
          }
        ...
        for (var attrName in self.daoFactory.rawAttributes) {
          if (self.daoFactory.rawAttributes.hasOwnProperty(attrName)) {
            var definition = self.daoFactory.rawAttributes[attrName]
              ...
              , isJSON          = definition.type.toString() === DataTypes.JSON.type
